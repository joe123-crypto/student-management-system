...
export default function ActionCard({
  ...
  return (
    <div className="group relative overflow-hidden rounded-[2rem] bg-[#1a1b3a] p-6 text-white shadow-[0_32px_64px_-16px_rgba(26,27,58,0.3)] sm:p-10">
          ...
          {items.length > 0 ? (
            items.map((item, idx) => (
              <div key={`${item}-${idx}`} className="flex items-start gap-3 group/item cursor-pointer">
                ...
                <p className="text-sm font-bold text-slate-300 transition-colors group-hover/item:text-white">{item}</p>
              </div>
            ))
          ) : (
